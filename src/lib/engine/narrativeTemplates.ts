...
export const NARRATIVE_TEMPLATES: NarrativeTemplate[] = [
  ...
  
  {
    ...
    headline: "Your cash is running low—you've got about a month left.",
    unseenRisk: "The situation is more urgent than it looks on paper. One late payment from a client or a surprise bill could force you into a corner.",
    contextualImpact: "At your current spending, the cash will last {{runwayDays}} days. If any client pays late, this could drop to just {{acceleratedDays}} days.",
    recommendation: "Action needed: hold off on any extra payments, try to get clients to pay you sooner, and look for a backup fund just in case.",
    riskLevel: 'critical',
  },
  
  {
    ...
    headline: "Your cash safety net is starting to shrink.",
    unseenRisk: "You're in a bit of a trap—you have enough for daily needs, but not enough if something goes wrong suddenly.",
    contextualImpact: "{{runwayDays}} days of cash might sound okay, but it only takes {{shockAbsorption}} unexpected problems to put you in trouble.",
    recommendation: "Try to build up a 3-month reserve. Cut back on things you don't strictly need right now and talk to vendors about better payment terms.",
    riskLevel: 'high',
  },
  
  {
    ...
    headline: "Project delays are slowly eating up your cash.",
    unseenRisk: "Every day a project is late, you're still paying out {{dailyBurn}} while waiting for {{blockedPayment}} to come in.",
    contextualImpact: "These delays have already cost you {{consumedDays}} days of cash runway. If this keeps up, the pressure will only grow.",
    recommendation: "Focus all energy on finishing {{projectName}} so you can get paid. Consider getting extra help just to get it over the finish line.",
    riskLevel: 'high',
  },
  
  {
    ...
    headline: "You're spending money faster than before.",
    unseenRisk: "Your costs are growing faster than your income. What looks like growth might actually be risky overspending.",
    contextualImpact: "Spending went up by {{burnTrend}}% this month. If this continues, your cash will drop from {{currentRunway}} to {{projectedRunway}} days much sooner.",
    recommendation: "Check your last 30 days of bills. Separate what's 100% necessary from what's nice-to-have, and pause the latter now.",
    riskLevel: 'medium',
  },
  ...
  
  {
    ...
    headline: "Project delays are starting to choke your cash flow.",
    unseenRisk: "The cycle of 'finish work and get paid' is broken. Every late milestone holds back money you already planned to use.",
    contextualImpact: "The {{projectName}} delay has kept {{blockedAmount}} away from you for {{delayDays}} days now. That's money that could be working for you.",
    recommendation: "Get it done: add extra help to {{projectName}} specifically. The cost of help is much lower than the impact of not getting paid.",
    riskLevel: 'high',
  },
  ...
  
  {
    ...
    headline: "You're leaning a bit too much on one client.",
    unseenRisk: "You're over-dependent. If this one client leaves, it's an immediate crisis—yet you're spending all your time just keeping them happy.",
    contextualImpact: "{{topClient}} brings in {{concentrationPercent}}% of your money. If they delay payment or leave, you'd lose {{impactAmount}} instantly.",
    recommendation: "Try to find more clients. Set a goal: no single client should be more than 30% of your income. Start looking for new ones this month.",
    riskLevel: 'medium',
  },
  ...
  
  {
    ...
    headline: "The Double Trouble: Cash is low and projects are late.",
    unseenRisk: "This is the hardest place to be. You don't have enough cash to handle problems, and you're not finishing projects fast enough to get new money.",
    contextualImpact: "You only have {{runwayDays}} days of cash left, while {{delayCount}} projects are stuck. This is a survivor situation.",
    recommendation: "Emergency mode: stop all non-urgent work, call every client asking for early payment, and cut your spending by 30% today.",
    riskLevel: 'critical',
  },
...
const TEMPLATE_TRANSLATIONS: Partial<Record<NarrativeLanguage, Record<string, Pick<NarrativeTemplate, 'headline' | 'unseenRisk' | 'contextualImpact' | 'recommendation'>>>> = {
  id: {
    'cash-01': { headline: 'Uang Anda sisa sedikit—kemungkinan hanya cukup buat sebulan lagi.', unseenRisk: 'Kondisinya lebih darurat daripada yang terlihat di laporan. Satu pembayaran klien yang telat atau ada tagihan mendadak bisa bikin Anda kesusahan.', contextualImpact: 'Dengan pengeluaran sekarang, uang Anda habis dalam {{runwayDays}} hari. Kalau ada klien telat bayar, ini bisa turun jadi cuma {{acceleratedDays}} hari saja.', recommendation: 'Segera: tunda dulu bayar yang tidak penting, minta klien bayar lebih cepat, dan cari dana cadangan buat jaga-jaga.' },
    'cash-02': { headline: 'Tabungan cadangan Anda mulai menipis.', unseenRisk: 'Ini situasi yang agak menjebak—uang cukup buat harian, tapi tidak cukup kalau tiba-tiba ada masalah besar.', contextualImpact: 'Punya napas buat {{runwayDays}} hari memang terdengar lumayan, tapi biasanya cuma kuat nahan {{shockAbsorption}} masalah sekaligus sebelum jadi kacau.', recommendation: 'Coba kumpulkan cadangan kas buat 3 bulan. Kurangi pengeluaran yang tidak wajib dan coba nego lagi sama supplier soal cara bayarnya.' },
    ...
    'revenue-04': { headline: 'Hati-hati, Anda terlalu bergantung pada satu klien.', unseenRisk: 'Terlalu riskan. Kalau satu klien ini pergi, Anda bisa langsung krisis—padahal waktu Anda habis cuma buat ngurusin dia saja.', contextualImpact: '{{topClient}} menyumbang {{concentrationPercent}}% pemasukan. Kalau dia telat bayar atau berhenti, Anda rugi {{impactAmount}} seketika.', recommendation: 'Coba mulai cari klien baru. Targetnya: jangan ada satu klien yang pegang lebih dari 30% pendapatan Anda. Fokus cari prospek baru bulan ini.' },
    ...
    'compound-01': { headline: 'Dobel masalah: uang makin menipis dan proyek pada telat.', unseenRisk: 'Ini situasi yang paling berat. Anda tidak punya cukup uang buat nahan guncangan, tapi proyek juga belum kelar-kelar buat dapat uang baru.', contextualImpact: 'Sisa uang cuma buat {{runwayDays}} hari, sementara ada {{delayCount}} proyek yang macet. Ini kondisi darurat.', recommendation: 'Langkah darurat: hentikan semua kerjaan yang tidak penting, telepon semua klien buat minta bayar lebih cepat, dan potong biaya 30% hari ini juga.' },
