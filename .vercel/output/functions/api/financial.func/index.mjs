...
function runSimulationMatch(inputs, seedVariance = 0.2) {
  ...
  for (let i = 1; i <= monthsToSimulate; i++) {
    const fragilityFactor = currentCash < inputs.emergencyBufferSize ? 1 + Math.max(0, inputs.emergencyBufferSize - currentCash) / (inputs.emergencyBufferSize || 1) : 1;
    ...
    const costSpikeMultiplier = Math.random() < 0.05 ? 1.2 * fragilityFactor : 1;
    ...
    if (inputs.activeProjectDelayDays && inputs.activeProjectDelayDays > 0) {
      ...
      if (remainingDelayDays > 0) {
        ...
        totalExpenses += opportunityCost;
        const totalDaysElapsedSinceStart = inputs.activeProjectDelayDays;
...
// src/lib/engine/storytelling.ts
function generateDecisionStory(inputs) {
...
// src/lib/engine/riskModel.ts
var NUM_SIMULATIONS = 500;
function evaluateFinancialRisk(inputs) {
  let survivedCount = 0;
  let totalRunway = 0;
